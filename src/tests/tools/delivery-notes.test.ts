...
describe('delivery-notes tool registry', () => {
  ...
  describe('lexware_download_delivery_note_file', () => {
    ...
    it('threads format="xml" to the application/xml Accept and yields delivery-note.xml on XML contentType', async () => {
      mockLexwareDownload.mockResolvedValue({
        fileName: undefined,
      ...
      const tools = await loadAndRegister();
      const dl = getTool(tools, 'lexware_download_delivery_note_file');
      const result = (await dl.handler({ id: 'dn-x', format: 'xml' })) as {
        structuredContent: { fileName: string };
      };
      expect(mockLexwareDownload).toHaveBeenCalledExactlyOnceWith(
        '/delivery-notes/dn-x/file',
        'application/xml',
      );
      expect(result.structuredContent.fileName).toBe('delivery-note.xml');
    });
  });
