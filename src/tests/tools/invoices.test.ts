...
function loadAndRegister() {
  return registerAndCapture(registerInvoiceTools as (s: unknown) => void);
}
...
describe('invoices tool registry', () => {
  ...
  it('registers exactly the expected 5 invoice tools (no finalize_invoice)', () => {
    const tools = loadAndRegister();
  ...
  describe('lexware_create_invoice', () => {
    ...
    it('declares a finalize boolean param in its input schema', () => {
      const tools = loadAndRegister();
      const create = getTool(tools, 'lexware_create_invoice');
  ...
  describe('lexware_get_invoice', () => {
    it('GETs /invoices/{id}', async () => {
      ...
      // Source calls lexwareRequest('GET', url) with only 2 args (no body/params
      // args at all) — expectRequest's DSL always supplies 4, which would assert
      // a call shape this source never makes. Match articles.test.ts's convention
      // for the same 2-arg GET-by-id shape.
  ...
  describe('lexware_download_invoice_file', () => {
    it('declares an optional format param in its input schema', () => {
      const tools = loadAndRegister();
  ...
  describe('lexware_pursue_invoice', () => {
    ...
    it('POSTs to /invoices with precedingSalesVoucherId and body, no finalize when omitted', async () => {
      ...
      expectRequest(mockLexwareRequest, {
        ...
        url: '/invoices',
        body: { foo: 'bar' },
        params: { precedingSalesVoucherId: '58e512ce-ea13-11eb-bac8-2f511e28942a' },
      });
    });
    ...
    it('declares precedingSalesVoucherId + body + finalize in its input schema', () => {
      const tools = loadAndRegister();
      const pursue = getTool(tools, 'lexware_pursue_invoice');
